  ...
  export default function reducer(state = initialState, action) {
    ...
    switch (type) {
      ...
      case ADD_POST:
        return {
          ...state,
          posts: [payload, ...state.posts],//huwi jib lpayload whatun belstate ba3den jeb lstate meshen ma etar a3ml refresh la shu lnew post deghre yendaf
          loading: false
        };
      case DELETE_POST:
        return {
          ...state,
          posts: state.posts.filter(post => post._id !== payload),//nafes mabda2 lAddPost
          loading: false
        };
      case POST_ERROR:
        return {
      ...
      case UPDATE_LIKES:
        return {
          ...state,
          posts: state.posts.map(post => post._id === payload.id ? {...post, likes: payload.likes} : post),//b3adel belike eza la bred lpost metl ma huwi
          loading: false
        };
      case ADD_COMMENT:
        return {
